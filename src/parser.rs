...
impl Parser {
    ...
    fn class_declaration(&mut self) -> Result<Stmt, String> {
        ...
        self.consume(OpenBrace, "Expected '{' before class body.")?;

        let mut methods = vec![];
        while !self.check(CloseBrace) && !self.is_at_end() {
            let method = self.function(FunctionKind::Method)?;
            methods.push(Box::new(method));
        }

        self.consume(CloseBrace, "Expected '}' after class body.")?;
    ...
    fn function(&mut self, kind: FunctionKind) -> Result<Stmt, String> {
        ...
        self.consume(OpenParen, &format!("Expected '(' after {kind:?} name"))?;
        ...
        self.consume(CloseParen, "Expected ')' after parameters.")?;
    ...
    fn for_statement(&mut self) -> Result<Stmt, String> {
        // for v
        //       ( SMTH ; SMTH ; SMTH )
        self.consume(OpenParen, "Expected '(' after 'for'.")?;

        // Consumes "SMTH ;"
        let initializer;
        if self.match_token(Semicolon) {
            initializer = None;
        } else if self.match_token(Local) {
        ...
        self.consume(OpenBrace, "Expected {' after loop condition.")?;
        ...
        self.consume(CloseParen, "Expected ')' after for clauses.")?;
        ...
        self.consume(OpenBrace, "Expected '}' after loop.")?;
        Ok(body)
    }

    fn while_statement(&mut self) -> Result<Stmt, String> {
        self.consume(OpenParen, "Expected '(' after 'while'")?;
        let condition = self.expression()?;
        self.consume(CloseParen, "Expected ')' after condition.")?;
    ...
    fn if_statement(&mut self) -> Result<Stmt, String> {
        self.consume(OpenParen, "Expected '(' after 'if'")?;
        let predicate = self.expression()?;
        self.consume(CloseParen, "Expected ')' after if-predicate")?;
    ...
    fn block_statement(&mut self) -> Result<Stmt, String> {
        ...
        self.consume(CloseBrace, "Expected '}' after a block")?;
    ...
    fn function_expression(&mut self) -> Result<Expr, String> {
        let paren = self.consume(OpenParen, "Expected '(' after anonymous function")?;
        let mut parameters = vec![];
        if !self.check(CloseParen) {
            loop {
                if parameters.len() >= 255 {
    ...
    fn equality(&mut self) -> Result<Expr, String> {
        ...
        while self.match_tokens(&[TildeEqual, EqualEqual]) {
            let operator = self.previous();
    ...
    fn unary(&mut self) -> Result<Expr, String> {
        if self.match_tokens(&[Tilde, Minus]) {
            let op = self.previous();
            let rhs = self.unary()?;
            Ok(Unary {
                id: self.get_id(),
    ...
    fn synchronize(&mut self) {
        ...
        while !self.is_at_end() {
            ...
            match self.peek().token_type {
                Function | Local | For | If | While | Print | Return => return,
                _ => (),
            }
